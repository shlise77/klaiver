		...
		<div id="paper">
			...
			<div id="individual_profile">
				<c:forEach items="${IndividualVO}" var="IndividualVO">
					...
					<div id="individual_info">
						<!--개인정보-->
						<input type="hidden" id="id" value="${sessionScope.login}">
						<input type="hidden" name=email value="${IndividualVO.email}">
						<div id="individual_name">${IndividualVO.first}
							${IndividualVO.last} ( ${IndividualVO.email} ) <a
								href="../member/individual_update"><img
								src="../resources/images/set.png" /></a>
						</div>
						<div id="individual_type">${IndividualVO.company}(
							${IndividualVO.title} )</div>
				...
				<c:forEach items="${IndividualVO}" var="IndividualVO">
					<div id="follow_btn">
							<button id="message" name="message"
								onclick="location.href='../notes/senderlist?userid=${sessionScope.login}'">Message</button>
						
						
						<p>
						New Message: <span id="follow_btn">${getlistcount.count}</span>개
						</p>
						<!-- 	<button type="submit" id="message" name="message" onclick="location.href='../'">Message</button> -->
					</div>
					<div id="follow_num_home">
						<!--팔로워/팔로잉-->
						<p>
							개인 팔로워 <span id="follower_num_home"> <a href="#" onclick="viewfollower('${IndividualVO.email}');">${followercount}</a> </span>명
						</p>
						<p>
							개인 팔로잉 <span id="following_num_home"> <a href="#" onclick="viewfollowing('${IndividualVO.email}');">${followingcount}</a> </span>명
						</p>
					</div>
				</c:forEach>
				</div>
			...
			<div id="contents_wrap">
					...
					<div id="_intro">
						...
						<c:forEach items="${IndividualVO}" var="IndividualVO">
							<div class="title_box_">
								자기소개<input type="hidden" name="email" id="userid"
							...
							<div id="personal_intro">
								...
								<c:if test="${empty IndividualVO.introduce}">
									...
									<textarea id="intro_edit" wrap="hard" name="introduce" onkeyup="resize(this)"></textarea>
									<button type="submit" id="edit_save" name="edit_save">저장</button>
								</c:if>
							</div>
						</c:forEach>
					</div>
				...
				<div id="share_info">
								...
								<ul class="title_box">
									<c:forEach items="${sessionidName}" var="sessionidName">
										<li class="name"><input type="hidden" name="warea"
											...
											name="userid" id="userid" value="${sessionScope.login}" />
					...
					<!--등록된 컨텐츠 박스-->
					<c:forEach items="${scrolllist}" var="ScrollBoardVO" varStatus="status">
						<div class="data_box">
							<ul class="title_box">
								<li>
								...
								<li class="name">
									<span class="personal_img"><img src="../resources/img/${ScrollBoardVO.profimg}"
										onclick="doImgPop('../resources/img/${ScrollBoardVO.profimg}')" /></span>
									<a href="../member/individualfollow?email=${ScrollBoardVO.userid}">${ScrollBoardVO.company}
										...
										<button class="enter" name="edit_save"
											onclick="remove(${ScrollBoardVO.sbid})">삭제</button>
							...
							<form class="repForm"id="repForm" method="POST" action="../member/reply">
								<ul class="title_box">
									...
									<li>
										<button class="comment_enter replytextbtn" name="edit_save" id="repsubmit">
